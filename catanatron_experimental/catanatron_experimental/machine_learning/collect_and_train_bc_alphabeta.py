"""
Behavioral Cloning: collect expert data from AlphaBeta + train policy network.

Identical to collect_and_train_bc.py except the expert teacher is
AlphaBetaPlayer(depth=2) instead of VFP's 1-ply argmax.
"""
...
def collect_expert_data(num_games, data_dir):
    """Play games using AlphaBeta's decisions, save chunks to disk."""
    print(f"\n{'='*60}")
    print(f"COLLECTING EXPERT DATA ({num_games} games) — AlphaBeta teacher")
    ...
    teacher = AlphaBetaPlayer(Color.BLUE)
    value_fn = base_fn()
    ...
    for game_i in range(games_already, num_games):
        ...
        while not done:
            ...
            # AlphaBeta picks the best action
            best_action = teacher.decide(game, playable_actions)
        ...
        # Heartbeat: print every 100 games so the log has a pulse
        if (game_i + 1) % 100 == 0:
            elapsed = time.time() - t0
            games_done = game_i + 1 - games_already
            rate = games_done / max(elapsed, 1e-6)
            eta_h = (num_games - game_i - 1) / max(rate, 1e-9) / 3600
            print(
                f"  [hb] game {game_i+1}/{num_games} | "
                f"{total_decisions} decs | "
                f"{rate*3600:.0f} games/h | ETA {eta_h:.1f}h | "
                f"wr {wins/(game_i+1)*100:.1f}%",
                flush=True,
            )
    ...
    print(f"  AlphaBeta win rate: {wins/num_games*100:.1f}%")
...
def train_bc(data_dir):
    ...
    model_path = os.path.join(script_dir, "model_bc_ab")
...
def main():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.join(script_dir, "bc_data_ab")
    ...
    print(f"  Output:     model_bc_ab.zip")
    print(f"  Next:       python train_ppo_agent_38.py")
    print(f"{'='*60}\n")
