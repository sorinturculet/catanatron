"""
V15: Production-Aware Value-Weighted Imitation

Same as v13 (ValueWeightedWrapper) but with production_aware_rewards instead of
dense_vp_rewards. This adds +0.02 per production point gained to the reward signal,
teaching the value function that expansion-oriented states are higher value.

Hypothesis: The PPO agent under-expands (1.17 settlements vs VFP's 3.17) because
dense_vp_rewards gives the same +0.1 for building on a high-production hex vs zero.
production_aware_rewards aligns the critic with VFP's own evaluation, which weights
production at 1e8 (second only to VP at 3e14).

Untested combination: v7 tested production_aware_rewards WITHOUT expert guidance (5.2%).
v13 tested ValueWeightedWrapper WITHOUT production rewards (13.0%).
V15 combines both for the first time.
"""
...
def main():
    ...
    # ValueWeightedWrapper parameters (same as v13)
    ...
    # Reward function: production_aware_rewards (THE key change from v13)
    # Adds +0.02 per production point gained on top of VP delta and win/loss
    ...
    experiment_name = f"ppo_v15-ProdValueWeighted-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    config = {
        ...
        "seed": seed,
        "version": "v15_prod_value_weighted",
        "approach": "production_aware_value_imitation",
    ...
    model_path = os.path.join(script_dir, "model_v15")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 80)
    print("MODEL LOADING (V15: PRODUCTION-AWARE VALUE-WEIGHTED IMITATION)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TRAINING CONFIGURATION (V15: PRODUCTION-AWARE VALUE-WEIGHTED IMITATION)")
    print("=" * 80)
    print(f"Approach:                Production-Aware Value-Weighted Imitation")
    ...
    print("=" * 80)
    print("STARTING TRAINING (V15: PRODUCTION-AWARE VALUE-WEIGHTED IMITATION)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TRAINING COMPLETED (V15: PRODUCTION-AWARE VALUE-WEIGHTED IMITATION)")
    print("=" * 80)
