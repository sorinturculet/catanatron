"""
V17: Adaptive Expert Curriculum

Same base as v15 (production_aware_rewards + expert guidance) but replaces
the fixed step-based decay with performance-gated adaptive decay.

Key changes from v15:
- AdaptiveCurriculumWrapper instead of ValueWeightedWrapper
- Expert bonus decays faster when agent is more competent (rolling normalized_score)
- min_scale=0.0 allows full graduation from expert (v15 floors at 0.05)
- Warmup at 2M steps (v15: 5M fixed), then adaptive from there

Phases:
1. Warmup (0-2M): Full expert guidance (bonus_scale=0.2)
2. Adaptive (2M-~6M): Decay rate = base_rate^(1 + 3.0 * competency)
3. Graduated (~6M-10M): Pure RL, no expert. Agent develops own strategies.

The thesis insight: don't dilute the expert (v16 failed at 11.6%).
Instead, learn when to let go. Agent needs independence to surpass the expert.
"""
...
from adaptive_curriculum_wrapper import AdaptiveCurriculumWrapper
...
class CurriculumLoggingCallback(BaseCallback):
    """Callback to log adaptive curriculum metrics to WandB."""
    ...
    def _on_step(self):
        if self.num_timesteps % self.log_freq == 0:
            try:
                ...
                if infos:
                    # Expert guidance metrics (same as v15)
                    bonuses = [
                    ...
                    competencies = [
                        info.get('curriculum_competency', 0)
                        for info in infos
                        if 'curriculum_competency' in info
                    ]
                    progresses = [
                        info.get('curriculum_progress', 0)
                        for info in infos
                        if 'curriculum_progress' in info
                    ]
                    decay_rates = [
                        info.get('curriculum_effective_decay', 0)
                        for info in infos
                        if 'curriculum_effective_decay' in info
                    ]
                    phases = [
                        info.get('curriculum_phase_num', 0)
                        for info in infos
                        if 'curriculum_phase_num' in info
                    ]
                    ...
                    # Curriculum metrics
                    if competencies:
                        log_dict['curriculum/competency'] = np.mean(competencies)
                    if progresses:
                        log_dict['curriculum/progress'] = np.mean(progresses)
                    if decay_rates:
                        log_dict['curriculum/effective_decay_rate'] = np.mean(decay_rates)
                    if phases:
                        log_dict['curriculum/phase'] = np.mean(phases)
...
def main():
    ...
    # AdaptiveCurriculumWrapper parameters
    bonus_scale = 0.2              # Initial max bonus (same as v15)
    warmup_steps = 2_000_000       # Total warmup steps before adaptive decay
    base_decay_rate = 0.99999      # Same base rate as v15
    competency_multiplier = 3.0    # 3x faster decay at max competency
    performance_window = 50_000    # Rolling buffer per env
    min_scale = 0.0                # Full graduation (v15: 0.05 floor)
    min_samples = 10_000           # Buffer must fill before adaptive kicks in
    graduation_threshold = 0.001   # Below this = graduated
    ...
    experiment_name = f"ppo_v17-AdaptiveCurriculum-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    config = {
        ...
        "seed": seed,
        "version": "v17_adaptive_curriculum",
        "approach": "adaptive_expert_curriculum",
        # AdaptiveCurriculumWrapper params
        "bonus_scale": bonus_scale,
        "warmup_steps": warmup_steps,
        "base_decay_rate": base_decay_rate,
        "competency_multiplier": competency_multiplier,
    ...
    # Per-env warmup: each subprocess wrapper tracks its own steps
    per_env_warmup = warmup_steps // n_envs
    ...
    model_path = os.path.join(script_dir, "model_v17")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 80)
    print("MODEL LOADING (V17: ADAPTIVE EXPERT CURRICULUM)")
    print("=" * 80)
    ...
    if not model_loaded:
        ...
        print(f"  Competency multiplier: {competency_multiplier}")
    ...
    print("\n" + "=" * 80)
    print("TRAINING CONFIGURATION (V17: ADAPTIVE EXPERT CURRICULUM)")
    print("=" * 80)
    print(f"Approach:                Adaptive Expert Curriculum")
    ...
    print(f"\nAdaptive Expert Curriculum:")
    print(f"  - Initial bonus:       {bonus_scale:>15}")
    print(f"  - Warmup steps:        {warmup_steps:>15,} total ({per_env_warmup:,}/env)")
    print(f"  - Base decay rate:     {base_decay_rate:>15}")
    print(f"  - Competency mult:     {competency_multiplier:>15}")
    print(f"  - Performance window:  {performance_window:>15,}/env")
    print(f"  - Min scale:           {min_scale:>15} (full graduation!)")
    print(f"  - Min samples:         {min_samples:>15,}")
    print(f"  - Graduation thresh:   {graduation_threshold:>15}")
    ...
    curriculum_callback = CurriculumLoggingCallback(log_freq=10000)
    callback = CallbackList([checkpoint_callback, curriculum_callback])

    print("=" * 80)
    print("STARTING TRAINING (V17: ADAPTIVE EXPERT CURRICULUM)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TRAINING COMPLETED (V17: ADAPTIVE EXPERT CURRICULUM)")
    print("=" * 80)
