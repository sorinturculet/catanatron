"""
V18: Extended V15 Training (20M steps)

Continues v15's proven recipe (production_aware_rewards + ValueWeightedWrapper)
for 20M total steps instead of 10M. Loads from the v15 trained model.

Key changes from v15:
- total_timesteps = 20M (double v15)
- Loads from model_v15 checkpoint (priority over experiment checkpoints)
- Expert bonus starts at floor (0.05) — matching where v15 ended
  (decay_start_step=0 so it starts at floor immediately)
- Lower learning rate for fine-tuning: 1e-4 → 1e-5 (vs v15's 3e-4 → 3e-5)

Why: WandB showed v15's normalized_score still rising at 10M (0.39→0.77).
The agent had only ~2.8M steps at the expert floor. V18 gives it 12.8M more
steps at the floor to develop independent strategies.

Same architecture, same reward function, same everything else.
"""
...
def main():
    # ===== V18: Extended V15 (20M steps) =====
    total_timesteps = 10_000_000  # This run trains 10M MORE steps on top of v15's 10M
    ...
    # Lower learning rate for fine-tuning (v15 used 3e-4 → 3e-5)
    initial_lr = 1e-4
    final_lr = 1e-5
    ...
    # ValueWeightedWrapper: start at the floor where v15 ended
    # bonus_scale=0.05, decay_start very high so it never decays further
    bonus_scale = 0.05             # Start at v15's floor value
    decay_start_step = 999_999_999 # Never decay (already at floor)
    decay_rate = 0.99999           # Doesn't matter (never reached)
    min_scale = 0.05               # Same floor as v15

    vps_to_win = 10
    env_name = "catanatron_gym:catanatron-v1"
    map_type = "BASE"

    # Opponent (same as v15)
    enemies = [ValueFunctionPlayer(Color.RED)]

    # Reward function (same as v15)
    ...
    experiment_name = f"ppo_v18-ExtendedV15-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    config = {
        ...
        "total_timesteps_including_v15": 20_000_000,
        "net_arch": net_arch,
        ...
        "seed": seed,
        "version": "v18_extended_v15",
        "approach": "extended_production_aware_value_imitation",
        "base_model": "model_v15 (10M steps)",
    ...
    model_path = os.path.join(script_dir, "model_v18")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 80)
    print("MODEL LOADING (V18: EXTENDED V15)")
    print("=" * 80)

    # Priority 1: Resume from v18 checkpoint (if we already started v18)
    ...
    if latest_checkpoint:
        print(f"\n✓ Found v18 checkpoint at {checkpoint_timesteps:,} timesteps")
        ...
        try:
            ...
            model_loaded = True
            print(f"✓ Successfully loaded v18 checkpoint!")
            print(f"  Resuming from timestep {timesteps_completed:,}")
        except Exception as e:
            print(f"✗ Failed to load v18 checkpoint: {e}")

    # Priority 2: Load from v15 trained model (bootstrap)
    if not model_loaded and os.path.exists(model_v15_path + ".zip"):
        print(f"\n→ Loading v15 trained model as starting point...")
        ...
        try:
            ...
            model._setup_lr_schedule()
            model_loaded = True
            print(f"✓ Successfully loaded v15 model!")
            print(f"  Training 10M more steps with lower lr ({initial_lr} → {final_lr})")
            print(f"  Expert bonus constant at floor: {bonus_scale}")
        except Exception as e:
            print(f"✗ Failed to load v15 model: {e}")

    # Priority 3: Fresh model (fallback — shouldn't happen)
    if not model_loaded:
        print(f"\n⚠ WARNING: No v15 model found! Training from scratch.")
        print(f"  This is not intended for v18. Check that model_v15.zip exists.")
    ...
    print("\n" + "=" * 80)
    print("TRAINING CONFIGURATION (V18: EXTENDED V15)")
    print("=" * 80)
    print(f"Approach:                Extended V15 (10M more steps)")
    print(f"Base model:              model_v15 (10M steps trained)")
    ...
    print(f"Total with v15:          {20_000_000:>15,}")
    ...
    print(f"  (lower than v15's 3e-4 → 3e-5 for fine-tuning)")
    ...
    print("=" * 80)
    print("STARTING TRAINING (V18: EXTENDED V15)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("TRAINING COMPLETED (V18: EXTENDED V15)")
    print("=" * 80)
