...
def main():
    ...
    experiment_name = f"ppo_v2-{selfplay}-{normalized}-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    model_path = os.path.join(script_dir, "model_v2")
    checkpoint_dir = "./logs/"
    ...
    # Priority 2: Load from model_v2 (if checkpoint failed)
    if not model_loaded:
        ...
        if os.path.exists(model_path + ".zip"):
            try:
                ...
                model._setup_lr_schedule()
                model_loaded = True
                print(f"✓ Successfully loaded base model: model_v2")
