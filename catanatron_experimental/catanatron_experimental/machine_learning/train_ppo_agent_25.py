"""
V25: Fresh 35M Run — v25 base + Road Bonus (no heuristic placement)

v24 (5.6%) proved that overriding initial placement with a heuristic breaks training.
v25 isolates the road bonus from v24's reward function without the placement wrapper.

Changes from v25:
  1. road_settlement_aware_rewards: settle +0.10 (was +0.08), road +0.04 (NEW)
  2. Everything else identical to v25 (4-stage LR, 35M fresh, same expert guidance)

Motivation: v25 (19.0%) has AVG ROAD 0.02 — almost never builds roads.
Road bonus should incentivize expansion chains toward new settlement spots.
"""
...
def main():
    # ===== V25: Fresh 35M with 4-Stage LR + Settlement Bonus =====
    ...
    experiment_name = (
        f"ppo_v25-RoadSettle-{iters}-{batch_size}-{gamma}"
    ...
    config = {
        "version": "v25_road_settlement_bonus",
        "approach": "fresh_4stage_lr_road_settlement_aware",
        ...
        "fresh_start": True,
        "motivation": "v23 AVG ROAD 0.02; add road +0.04 and bump settle +0.10 to incentivize expansion",
    }
    run = wandb.init(project="catanatron", config=config, sync_tensorboard=True, name="v25")
    ...
    model_path     = os.path.join(script_dir, "model_v25")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 70)
    print("MODEL LOADING (V25: FRESH 4-STAGE LR + SETTLEMENT BONUS)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("TRAINING CONFIGURATION (V25: FRESH 4-STAGE LR + SETTLEMENT BONUS)")
    print("=" * 70)
    ...
    print(f"  Road built:         +0.04     (NEW — expansion incentive)")
