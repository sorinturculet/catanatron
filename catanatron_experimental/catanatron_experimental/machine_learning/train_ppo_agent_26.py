"""
V26: Fresh 50M — v25 recipe (road+settlement rewards) with 5-stage LR schedule

v25 (28.4%) proved road_settlement_aware_rewards is the best reward function,
but had a LR schedule bug (ended at 7.9e-5 instead of 1e-5 due to checkpoint resume).

v26 runs the same recipe from scratch for 50M steps with a proper 5-stage LR:
  Stage 1 ( 0–10M): 3e-4 → 3e-5   (exploration)
  Stage 2 (10–20M): 1e-4 → 1e-5   (reset, refine)
  Stage 3 (20–30M): 5e-5 → 1e-5   (reset, polish)
  Stage 4 (30–40M): 3e-5 → 1e-5   (fine-tune)
  Stage 5 (40–50M): 2e-5 → 5e-6   (final polish)

Everything else identical to v25: same architecture, same reward, same expert guidance.
"""
...
def five_stage_lr_schedule(total_timesteps):
    """
    5-stage piecewise LR for 50M steps.

    Stage 1 ( 0–10M): 3e-4 → 3e-5   progress 1.0  → 0.8
    Stage 2 (10–20M): 1e-4 → 1e-5   progress 0.8  → 0.6
    Stage 3 (20–30M): 5e-5 → 1e-5   progress 0.6  → 0.4
    Stage 4 (30–40M): 3e-5 → 1e-5   progress 0.4  → 0.2
    Stage 5 (40–50M): 2e-5 → 5e-6   progress 0.2  → 0.0
    """
    # Breakpoints (progress_remaining values)
    p1 = 0.8   # 10M / 50M
    p2 = 0.6   # 20M / 50M
    p3 = 0.4   # 30M / 50M
    p4 = 0.2   # 40M / 50M
    ...
    def lr_schedule(progress_remaining):
        ...
        elif progress_remaining > p4:
            frac = (progress_remaining - p4) / (p3 - p4)
            return 1e-5 + (3e-5 - 1e-5) * frac
        else:
            frac = progress_remaining / p4
            return 5e-6 + (2e-5 - 5e-6) * frac
        ...
        if self.num_timesteps % self.log_freq == 0:
            try:
                ...
                if infos:
                    ...
                    elif self.num_timesteps < 40_000_000:
                        log_dict["train/lr_stage"] = 4
                    else:
                        log_dict["train/lr_stage"] = 5
...
def main():
    ...
    # Architecture — same as v25 (proven)
    ...
    lr_schedule = five_stage_lr_schedule(total_timesteps)

    experiment_name = "ppo_v26-Fresh50M-5stageLR-RoadSettle"

    config = {
        "version": "v26_fresh_50M_5stage",
        "approach": "fresh_5stage_lr_road_settlement_aware",
        "total_timesteps": total_timesteps,
        "lr_stage1": "3e-4 → 3e-5  (0–10M)",
        "lr_stage2": "1e-4 → 1e-5  (10–20M, reset)",
        "lr_stage3": "5e-5 → 1e-5  (20–30M, reset)",
        "lr_stage4": "3e-5 → 1e-5  (30–40M, fine-tune)",
        "lr_stage5": "2e-5 → 5e-6  (40–50M, final polish)",
        ...
        "fresh_start": True,
        "motivation": "v25 recipe (28.4%) + more compute (50M) + proper 5-stage LR (v25 had schedule bug)",
    }
    run = wandb.init(project="catanatron", config=config, sync_tensorboard=True, name="v26")
    ...
    model_path     = os.path.join(script_dir, "model_v26")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 70)
    print("MODEL LOADING (V26: FRESH 50M — 5-STAGE LR)")
    print("=" * 70)
    ...
    if latest_checkpoint:
        ...
        try:
            ...
            stage = (1 if timesteps_completed < 10_000_000 else
                     ...
                     3 if timesteps_completed < 30_000_000 else
                     4 if timesteps_completed < 40_000_000 else 5)
    ...
    print("\n" + "=" * 70)
    print("TRAINING CONFIGURATION (V26: FRESH 50M — 5-STAGE LR)")
    print("=" * 70)
    ...
    print(f"\nLR Schedule (5-stage piecewise):")
    print(f"  Stage 1   0–10M:    3e-4 → 3e-5  (exploration)")
    print(f"  Stage 2  10–20M:    1e-4 → 1e-5  (reset, refine)")
    print(f"  Stage 3  20–30M:    5e-5 → 1e-5  (reset, polish)")
    print(f"  Stage 4  30–40M:    3e-5 → 1e-5  (fine-tune)")
    print(f"  Stage 5  40–50M:    2e-5 → 5e-6  (final polish)")
