"""
V27: Fresh 60M with fixed LR schedule + city bonus

v26 hit 31.2% (new best) but had an LR bug: SB3's progress_remaining resets
on checkpoint resume, so the 5-stage schedule never reached low LR (ended at
7.9e-5 instead of 5e-6). Metrics were still climbing at end of training.

v27 fixes the LR bug by converting progress_remaining to absolute timesteps
inside the schedule function. This makes the multi-stage schedule immune to
checkpoint resume. Also adds a small city upgrade bonus (+0.04) since v26
only built 1.27 cities vs VFP's 2.04.

4-stage LR (60M total):
  Stage 1 ( 0–15M): 3e-4 → 3e-5  (exploration)
  Stage 2 (15–30M): 1e-4 → 1e-5  (refine)
  Stage 3 (30–45M): 3e-5 → 5e-6  (fine-tune)
  Stage 4 (45–60M): 1e-5 → 2e-6  (final polish)

Everything else from v26: same architecture, same expert guidance (0.2→0.05).
"""
...
from reward_functions import city_road_settlement_aware_rewards, mask_fn
...
def main():
    ...
    lr_stages = [
        (0.00, 0.25, 3e-4, 3e-5),   # Stage 1 (0–15M): exploration
        (0.25, 0.50, 1e-4, 1e-5),   # Stage 2 (15–30M): refine
        (0.50, 0.75, 3e-5, 5e-6),   # Stage 3 (30–45M): fine-tune
        (0.75, 1.00, 1e-5, 2e-6),   # Stage 4 (45–60M): final polish
    ]
    ...
    reward_function = partial(city_road_settlement_aware_rewards, vps_to_win=vps_to_win)
    reward_function.__name__ = city_road_settlement_aware_rewards.__name__
    ...
    experiment_name = "ppo_v27-Fresh60M-FixedLR-CityBonus"

    config = {
        "version": "v27_fresh_60M_fixed_lr_city",
        "approach": "fresh 60M with absolute-step LR (bug fix) + city bonus",
        "total_timesteps": total_timesteps,
        "lr_stage1": "3e-4 → 3e-5  (0–15M, exploration)",
        "lr_stage2": "1e-4 → 1e-5  (15–30M, refine)",
        "lr_stage3": "3e-5 → 5e-6  (30–45M, fine-tune)",
        "lr_stage4": "1e-5 → 2e-6  (45–60M, final polish)",
        ...
        "reward_notes": "VP +0.10, OWS +0.02, BW +0.04, settle +0.10, road +0.04, city +0.04, win/loss +/-1.0",
    ...
    run = wandb.init(project="catanatron", config=config, sync_tensorboard=True, name="v27")
    ...
    model_path     = os.path.join(script_dir, "model_v27")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 70)
    print("MODEL LOADING (V27: FRESH 60M — FIXED LR + CITY BONUS)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("TRAINING CONFIGURATION (V27: FRESH 60M — FIXED LR + CITY BONUS)")
    print("=" * 70)
    ...
    print(f"  Stage 1   0–15M:    3e-4 → 3e-5  (exploration)")
    print(f"  Stage 2  15–30M:    1e-4 → 1e-5  (refine)")
    print(f"  Stage 3  30–45M:    3e-5 → 5e-6  (fine-tune)")
    print(f"  Stage 4  45–60M:    1e-5 → 2e-6  (final polish)")
    print(f"\nReward: {reward_function.__name__}")
    print(f"  VP delta:           +0.10 per VP")
    print(f"  OWS prod:           +0.02/pt  (ore/wheat/sheep)")
    print(f"  BW  prod:           +0.04/pt  (brick/wood — 2x)")
    print(f"  Settlement placed:  +0.10")
    print(f"  Road built:         +0.04")
    print(f"  City upgrade:       +0.04  (NEW in v27)")
