"""
V28: Expansion-heavy rewards + higher gamma to beat VFP

v27 hit 31.6% but still barely expands: 1.15 settlements, 0.12 roads vs
VFP's 3.22 settlements, 0.84 roads. The model wins through dev cards (1.80
dev VP) but leaves massive production gains on the table.

v28 changes:
  1. Heavily boosted expansion rewards:
     - Settlement: +0.10 → +0.25 (2.5x boost)
     - Road:       +0.04 → +0.12 (3x boost)
     - City:       +0.04 → +0.08 (2x boost)
  2. Higher gamma: 0.99 → 0.997 (value long-term expansion payoff)
  3. Same fixed LR schedule (absolute step counting, bug-free)
...
Same architecture, same expert guidance (0.2→0.05).
"""
...
def main():
    ...
    experiment_name = "ppo_v28-Fresh60M-ExpansionHeavy-Gamma997"

    config = {
        "version": "v28_fresh_60M_expansion_heavy",
        "approach": "fresh 60M with expansion-heavy rewards + gamma=0.997",
    ...
    run = wandb.init(project="catanatron", config=config, sync_tensorboard=True, name="v28")
    ...
    model_path     = os.path.join(script_dir, "model_v28")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 70)
    print("MODEL LOADING (V28: FRESH 60M — EXPANSION HEAVY + GAMMA 0.997)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("TRAINING CONFIGURATION (V28: FRESH 60M — EXPANSION HEAVY + GAMMA 0.997)")
    print("=" * 70)
    ...
    print(f"  Settlement placed:  +0.25  (was +0.10, 2.5x boost)")
    print(f"  Road built:         +0.12  (was +0.04, 3x boost)")
    print(f"  City upgrade:       +0.08  (was +0.04, 2x boost)")
    print(f"  Gamma:              {gamma}  (was 0.99 — longer horizon)")
