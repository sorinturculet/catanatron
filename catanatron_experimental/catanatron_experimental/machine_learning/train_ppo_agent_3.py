...
def main():
    # ===== Improved Hyperparameters (same as v2) =====
    # Reduced to 10M timesteps for faster iteration
    ...
    # Higher entropy for better exploration (fixed value, no scheduling)
    ent_coef = 0.02
    ...
    # NEW: Use dense VP-based reward function instead of sparse partial_rewards
    reward_function = partial(dense_vp_rewards, vps_to_win=vps_to_win)
    reward_function.__name__ = dense_vp_rewards.__name__
    representation = "mixed"

    # Core PPO parameters - optimized for sample efficiency
    gamma = 0.99
    gae_lambda = 0.95  # Added for better credit assignment
    normalized = False
    selfplay = False
    seed = 42

    # Parallelization - increase if cluster has more CPUs
    n_envs = 16  # Increased from 8 for better sample collection
    n_steps = 512  # Increased from 256 for longer rollouts
    batch_size = 512  # Smaller minibatches for more gradient updates
    ...
    experiment_name = f"ppo_v3-{selfplay}-{normalized}-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    model_path = os.path.join(script_dir, "model_v3")
    checkpoint_dir = "./logs/"
    ...
    # Priority 2: Load from model_v3 (if checkpoint failed)
    if not model_loaded:
        ...
        if os.path.exists(model_path + ".zip"):
            try:
                ...
                model._setup_lr_schedule()
                model_loaded = True
                print(f"✓ Successfully loaded base model: model_v3")
