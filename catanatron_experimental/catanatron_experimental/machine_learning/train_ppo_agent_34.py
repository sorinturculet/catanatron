"""
V35: PPO fine-tuning from Behavioral Cloning checkpoint (100K games).

Phase 2 of the BC → PPO pipeline.
  1. Load model_bc.zip (policy already plays like VFP, trained on 100K games)
  2. Fine-tune with PPO + minimal reward (VP delta + win/loss only)
  3. No expert guidance — agent explores freely beyond VFP
  4. Cosine LR decay (5e-5 → 2e-6 over 80M steps) — clean, paper-backed schedule
  5. ent_coef=0.03 — slightly more exploration than v34

LR schedule (cosine decay, 80M total):
  5e-5 → 2e-6 following cosine curve (SGDR, Loshchilov & Hutter 2016)
  Accounts for checkpoint offset so resume mid-cosine is correct.
"""
...
def make_cosine_lr_schedule(lr_start, lr_end, total_timesteps, offset):
    ...
    Based on SGDR (Loshchilov & Hutter, 2016). Decays from lr_start to lr_end
    following a cosine curve over total_timesteps steps. When resuming from a
    checkpoint at `offset` steps, the LR starts mid-cosine at the correct value
    rather than resetting to lr_start.
    """
...
def main():
    ...
    gamma      = 0.997
    gae_lambda = 0.95
    ent_coef   = 0.03  # slightly more exploration than v34 (0.02)
    n_envs     = 32
    n_steps    = 512
    batch_size = 1024
    n_epochs   = 4
    ...
    experiment_name = "ppo_v35-BC-finetune"

    config = {
        "version": "v35_bc_finetune",
        "approach": "Behavioral Cloning (100K games) → PPO fine-tune",
        "base_model": "model_bc.zip (BC on 100K VFP games)",
    ...
    run = wandb.init(project="catanatron", config=config, sync_tensorboard=True, name="v35")
    ...
    model_path     = os.path.join(script_dir, "model_v35")
    bc_path        = os.path.join(script_dir, "model_bc.zip")
    checkpoint_dir = "./logs/"

    print("\n" + "=" * 70)
    print("MODEL LOADING (V34: BC → PPO)")
    print("=" * 70)

    # Check for v35 checkpoint first (resume)
    ...
    if latest_checkpoint:
        print(f"\n  Found v35 checkpoint at {checkpoint_timesteps:,} steps")
        try:
            ...
            model.tensorboard_log = "./logs/mppo_tensorboard/v35"
    ...
    if not model_loaded and os.path.exists(bc_path):
        ...
        model_loaded = True
        print(f"  BC model loaded — fine-tuning with PPO")

    if not model_loaded:
        print(f"\n  ERROR: model_bc.zip not found at {bc_path}")
        print(f"  Run collect_and_train_bc.py first!")
        return

    remaining_timesteps = total_timesteps - timesteps_completed

    print(f"\n" + "=" * 70)
    print("TRAINING CONFIGURATION (V34: BC → PPO)")
    print("=" * 70)
    ...
    model.learn(total_timesteps=remaining_timesteps, callback=callback, tb_log_name="v35")
    model.save(model_path)

    elapsed = time.time() - start_time
    print("\n" + "=" * 70)
    print("TRAINING COMPLETED (V34 — BC → PPO)")
    print("=" * 70)
