"""
PPO Agent v5: Self-Attention Architecture

Based on v3 (best avg VP: 5.12, 9% win rate) with added self-attention mechanism
for global board reasoning.

Key changes from v3:
- CustomCNNWithAttention instead of CustomCNN
- Self-attention layer after CNN for spatial reasoning across entire board
- Same single ValueFunctionPlayer opponent (proven to produce best strategic depth)
- Same dense_vp_rewards (VP gain/loss + win/loss bonus)

Expected improvements:
- Better understanding of road networks, resource clusters, blocking strategies
- Improved city placement decisions
- Better army/longest road pursuit through global context
"""
...
from catanatron_experimental.machine_learning.custom_cnn import CustomCNNWithAttention
...
def main():
    # ===== V5: Same hyperparameters as v3 (best performing) =====
    total_timesteps = 10_000_000
    #total_timesteps = 100_000
    # Same network architecture as v3
    ...
    # V5: Same single strong opponent as v3 (produces best strategic depth)
    ...
    # ===== V5 NEW: Self-attention parameters =====
    num_attention_heads = 4
    attention_dropout = 0.1
    features_dim = 512  # Output dimension of feature extractor
    ...
    if representation == "mixed":
        arch_str = "Cnn" + "x".join([str(i) for i in cnn_arch]) + "+SelfAttn+" + arch_str
    enemy_desc = "".join(e.__class__.__name__ for e in enemies)
    experiment_name = f"ppo_v5_attention-{selfplay}-{normalized}-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    model_path = os.path.join(script_dir, "model_v5")
    checkpoint_dir = "./logs/"

    print("\n" + "="*80)
    print("MODEL LOADING - V5 SELF-ATTENTION")
    print("="*80)
    ...
    # Priority 2: Load from model_v5 (if checkpoint failed)
    if not model_loaded:
        ...
        if os.path.exists(model_path + ".zip"):
            try:
                ...
                model._setup_lr_schedule()
                model_loaded = True
                print(f"✓ Successfully loaded base model: model_v5")
    ...
    # Priority 3: Create new model with self-attention (if nothing else worked)
    if not model_loaded:
        print(f"\nCreating new model with SELF-ATTENTION from scratch...")
        ...
        if representation == "mixed":
            # V5: Use CustomCNNWithAttention instead of CustomCNN
            policy_kwargs["features_extractor_class"] = CustomCNNWithAttention
    ...
    print("\n" + "="*80)
    print("V5 TRAINING CONFIGURATION")
    print("="*80)
    ...
    print(f"\n*** V5 NEW: SELF-ATTENTION ARCHITECTURE ***")
    print(f"  Attention heads:       {num_attention_heads:>15}")
    print(f"  Attention dropout:     {attention_dropout:>15.2f}")
    ...
    print("="*80)
    print("STARTING V5 SELF-ATTENTION TRAINING")
    print("="*80)
