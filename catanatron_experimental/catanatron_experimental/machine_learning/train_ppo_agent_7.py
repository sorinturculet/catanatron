...
def main():
    # ===== V7 Hyperparameters =====
    # Key changes from v3:
    # - production_aware_rewards instead of dense_vp_rewards
    # - Higher entropy (0.03) for more exploration
    # - Lower learning rate (1e-4 -> 1e-5) for stability
    # - More epochs (6) for better gradient updates

    total_timesteps = 10_000_000  # Increased from 10M
    ...
    initial_lr = 1e-4  # Changed from 3e-4
    final_lr = 1e-5    # Changed from 3e-5

    # Higher entropy for better exploration
    ent_coef = 0.03  # Changed from 0.02
    ...
    # NEW: Use production-aware reward function
    ...
    experiment_name = f"ppo_v7-{selfplay}-{normalized}-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    config = {
        ...
        "seed": seed,
        "version": "v7_production_aware_rewards",
    }
    run = wandb.init(
        project="catanatron",
        config=config,
    ...
    model_path = os.path.join(script_dir, "model_v7")
    checkpoint_dir = "./logs/"
    ...
    # Priority 2: Load from model_v7 (if checkpoint failed)
    if not model_loaded:
        ...
        if os.path.exists(model_path + ".zip"):
            try:
                ...
                model._setup_lr_schedule()
                model_loaded = True
                print(f"✓ Successfully loaded base model: model_v7")
    ...
    if not model_loaded:
        ...
        print(f"  Reward function: {reward_function.__name__} (PRODUCTION-AWARE)")
    ...
    print("\n" + "="*80)
    print("TRAINING CONFIGURATION (V7)")
    print("="*80)
    ...
    # Save checkpoints more frequently for faster experimentation
    ...
    if selfplay:
        ...
        for i in range(selfplay_iterations):
            model.learn(
                total_timesteps=int(remaining_timesteps / selfplay_iterations),
                callback=callback,
            )
            model.save(model_path)
    else:
