...
def main():
    # ===== V8A: Stage 1 Bootstrap Training =====
    # Train against ValueFunctionPlayer to learn fundamentals
    # This is the foundation for Stage 2 self-play training

    # Reduced to 5M timesteps for faster iteration (Stage 1 only)
    total_timesteps = 5_000_000

    # Same network architecture as v3 (proven to work)
    ...
    # Learning rate - standard PPO range (same as v3)
    initial_lr = 3e-4
    final_lr = 3e-5
    ...
    # Use dense VP-based reward function (proven in v3)
    ...
    gae_lambda = 0.95
    normalized = False
    selfplay = False  # Stage 1: No self-play
    seed = 42
    ...
    experiment_name = f"ppo_v8a-{selfplay}-{normalized}-{iters}-{batch_size}-{gamma}-{enemy_desc}-{reward_function.__name__}-{representation}-{arch_str}-{initial_lr}lr-{vps_to_win}vp-{map_type}map"
    ...
    model_path = os.path.join(script_dir, "model_v8a")
    checkpoint_dir = "./logs/"

    print("\n" + "="*80)
    print("MODEL LOADING (STAGE 1: BOOTSTRAP)")
    print("="*80)
    ...
    # Priority 2: Load from model_v8a (if checkpoint failed)
    if not model_loaded:
        ...
        if os.path.exists(model_path + ".zip"):
            try:
                ...
                model._setup_lr_schedule()
                model_loaded = True
                print(f"✓ Successfully loaded base model: model_v8a")
    ...
    print("\n" + "="*80)
    print("TRAINING CONFIGURATION (STAGE 1: BOOTSTRAP)")
    print("="*80)
    print(f"Stage:                   Bootstrap (vs ValueFunctionPlayer)")
    ...
    print("\n" + "="*80)
    print("TRAINING COMPLETED (STAGE 1: BOOTSTRAP)")
    print("="*80)
    ...
    print(f"\n🎯 Next step: Run Stage 2 (train_ppo_agent_8b.py) for self-play training!")
    print("="*80 + "\n")
