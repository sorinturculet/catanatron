...
def buildable_node_values(game: Game, p0_color: Color):
    ...
    # NOTE: Use "BUILDABLE_" prefix (not "NODE") to avoid is_graph_feature() filtering
    for node_id in range(NUM_NODES):
        if node_id in buildable:
            total_prod = sum(catan_map.node_production[node_id].values())
            features[f"BUILDABLE_{node_id}_PRODUCTION"] = total_prod
        else:
            features[f"BUILDABLE_{node_id}_PRODUCTION"] = 0.0
